...
class StreamDeck extends EventEmitter {
  ...
  drawColor(hexColor, buttonNumber) {
    var colors = Jimp.intToRGBA(hexColor);
    ...
    var color = Buffer.from([ colors.g, colors.b, colors.a, 0xFF ]);
    var buffer = Buffer.alloc(ICON_SIZE * ICON_SIZE * 4, color);
  ...
  drawImageBuffer(imageBuffer, buttonNumber) {
    var data = pageData.getPageData(imageBuffer, buttonNumber);
